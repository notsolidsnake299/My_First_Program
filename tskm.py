...
def animation(stdscr):
    ...
    frames = [open, closed] # variable para la animacion
    blink_ms = 600 # variable para el parpadeo 
    i = 0
    last = time.monotonic()
...
#--- CONFIG PROCESS ---
    ...
    while True:
        ...
        if time.monotonic() - last_procc_upd > 2:
            processes = sorted(
        ...
        try:
            ...
            stdscr.addstr(y_pos, 40, "PID        Name                  Status        CPU %     Command ", curses.A_BOLD)
            
            for i, proc in enumerate(processes[scroll_offset:scroll_offset + 20]):
                ...
                command_text = ''.join(proc.info.get('cmdline', []))[:10]
                ...
                if isinstance(cpu_percent, (int, float)):
                    ...
                    cpu_str = " N/A "
                #--- PROCESS INFO --- #
                line_text = f"{pid:<10} {name:<22} {status:<11} {cpu_str} {command_text:<15}"
