...
def cpu_monitor(stdscr):
    ...
    while True:
        ...
        #processes
        for i, proc in enumerate(psutil.process_iter(['pid', 'name', 'status'])):
            if i >= 10:
                break
            y_pos = 2
            pid = proc.info['pid']
            ...
            y_pos += 1
            stdscr.addstr(y_pos, 40, f"PID: {pid:<7} | Name: {name:<20} | Status: {status}")
            stdscr.addstr(6,1, "up arrow and down arrow to move")
            stdscr.addstr(8, 1, "press q to exit")
